...
function InputModal({showModal, handleClose, dayData = "mon", startTimeData = 9, endTimeData = 10, lectureNameData = "", colorData = "#CEE5D0", idNum}) {
      ...
      <form onSubmit={handleSubmit(idNum? Edit :Submit)}>
        ...
        <DialogContent style={{width:"260px"}}>
          <Controller 
            ...
            render={({field})=>(
              <TextField 
                ...
                style={{marginTop:"30px", width:"260px"}}
                autoComplete="off"
                label="강의명"
              />
            )}
          ...
          {errors.lectureName?.type === 'required' && (
            <p style={{color:"#F65A83"}}>강의명을 입력해주세요.</p>
          )}
          ...
          <Stack spacing={3} style={{marginTop:"30px", width:"260px"}}>
            <Controller 
              control={control}
              name="startTime"
              rules={{required:true}}
              render={({field}) => (
                <TextField {...field} select error={!!errors.startTime || !!(errors.endTime?.type === 'validate')}
                  style={{marginTop:"30px", width:"260px"}}
                  label="시작 시간"
            ...
            {errors.startTime?.type === "required" && (
              <p style={{color:"#F65A83"}}>강의 시작 시간을 선택해주세요.</p>
            )}
            <Controller 
              control={control}
              name="endTime"
              ...
              render={({field}) => (
                <TextField {...field} select error={!!errors.endTime}
                  style={{marginTop:"30px", width:"260px"}}
                  label="종료 시간"
            ...
            {errors.endTime?.type === "required" && (
              <p style={{color:"#F65A83"}}>강의 종료 시간을 선택해주세요.</p>
            )}
            {errors.endTime?.type === "validate" && (
              <p style={{color:"#F65A83"}}>시작 시간과 종료 시간을 확인해주세요.</p>
            )}
          </Stack>
